  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-3">
        <h3 className="text-[17px] font-semibold tracking-tight">プロジェクト</h3>
        <button onClick={() => { setEditing(null); setOpen(true); }} className="text-[13.5px] font-medium px-2.5 py-1 bg-ink text-white rounded hover:bg-black">+ 追加</button>
      </div>
      {loading ? (
        <p className="text-ink-3 font-mono text-[13.5px]">// loading…</p>
      ) : projects.length === 0 ? (
        <p className="text-ink-3 font-mono text-[13.5px]">// プロジェクトはありません</p>
      ) : (
        <div className="border border-border rounded">
          {projects.map((p) => (
            <div key={p.id} className="flex items-center gap-3 px-3 py-2 border-b border-border-2 last:border-b-0">
              ...
              <button onClick={() => { setEditing(p); setOpen(true); }} className="text-[13px] text-ink-3 hover:text-ink px-1.5">編集</button>
              <button onClick={() => handleDelete(p)} className="text-[13px] text-ink-3 hover:text-crit px-1.5">削除</button>
            </div>
          ))}
        </div>
      )}
